...
export default function FreelancerSearch({ onHireFreelancer }: FreelancerSearchProps) {
  ...
  const fetchFreelancers = async (q?: string) => {
    try {
      ...
      const endpoint = token ? "/client/freelancers/search" : "/client/freelancers/public/search";
      const res = await axiosInstance.get(endpoint, {
        params: { searchTerm: q || undefined, page: 1, limit: 12 },
      ...
      const sorted = arr
        .map((f) => ({
          ...f,
          _rating: typeof f.stats?.avgRating === 'number'
            ? f.stats.avgRating
            : (typeof f.avgRating === 'number' ? f.avgRating : (typeof f.rating === 'number' ? f.rating : 0)),
        }))
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      ...
      try {
        ...
        const token = localStorage.getItem("token");
        const endpoint = token ? "/client/freelancers/suggest" : "/client/freelancers/public/suggest";
        const res = await axiosInstance.get(endpoint, {
          params: { q, limit: 6 },
          headers: token ? { Authorization: `Bearer ${token}` } : undefined,
        });
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex items-center space-x-2 mt-6 mb-2">
        <div className="relative flex-1">
          ...
          />
          {showSuggestions && (
            <div className="absolute z-20 top-full mt-1 w-full bg-card border rounded shadow-sm">
              {isSuggesting && suggestions.length === 0 ? (
                ...
                suggestions.map((s, idx) => (
                  <button
                    ...
                    onClick={() => {
                      if (s.id) {
                        window.location.href = `/client/freelancers/${s.id}`;
                        return;
                      }
                      const text = s.name || s.expertise || '';
                      setSearchQuery(text);
                      setShowSuggestions(false);
                      fetchFreelancers(text);
                    }}
                  >
                    <div className="font-medium truncate">{s.name || s.expertise}</div>
                    {s.name && s.expertise && (
                      <div className="text-xs text-muted-foreground truncate">{s.expertise}</div>
                    )}
                  </button>
                ))
              )}
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 gap-6 pb-12">
          {freelancers.map((f) => {
            ...
            const reviews = typeof f.stats?.reviewsCount === 'number' ? f.stats.reviewsCount : undefined;
            ...
            return (
              <Card key={f.id || f.uuid} className="hover:shadow-lg transition group">
                <CardContent className="p-6">
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    <Button 
                      className="flex-1"
                      onClick={() => onHireFreelancer && onHireFreelancer(f)}
                    >
                      Hire Now
                    </Button>
                  </div>
