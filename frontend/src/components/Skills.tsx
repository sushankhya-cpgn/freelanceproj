...
const Skills: React.FC = () => {
  ...
  // Scroll function (original logic preserved; note: may need refinement for smooth centering)
  const ScrollCen = () => {
    if (slider.current) {
  ...
  return (
    <div className="flex md:flex-row flex-col my-7 md:items-start items-center md:space-y-0 space-y-7 2xl:justify-around justify-between">
      ...
      <ul
        className="flex md:flex-col flex-row md:items-start items-center space-x-5 md:space-x-0 md:space-y-6 space-y-0 sm:w-auto sm:h-auto w-full h-full sm:overflow-hidden overflow-x-scroll scroll whitespace-nowrap scroll-smooth scrollbar-hide"
        ref={slider}
      >
        <li
          className={`font-semibold xl:text-4xl lg:text-3xl text-2xl ${
            linkI ? "text-[#0C4A6E]" : "text-gray-300"
          }`}
          onClick={scrollLeft}
        >
        ...
        <li
          className={`font-semibold xl:text-4xl lg:text-3xl text-2xl ${
            linkII ? "text-[#0C4A6E]" : "text-gray-300"
          }`}
          onClick={ScrollCen}
        >
          <span className="cursor-pointer" onClick={HandleLinkII}>
            Trending Skills {/* Fixed typo for clarity */}
          </span>
        </li>
        <li
          className={`font-semibold xl:text-4xl lg:text-3xl text-2xl ${
            linkIII ? "text-[#0C4A6E]" : "text-gray-300"
          }`}
          onClick={scrollRight}
        >
      ...
      {/* Content: Two columns (left/right) based on active tab */}
          ...
          <>
            <ul className="flex flex-col space-y-3">
              {ListI.left.map((curVal) => (
                <li
                  className="font-semibold xl:text-2xl lg:text-xl text-md text-zinc-500 hover:text-cyan-700" // Removed 'hidde'
                  key={curVal.id}
                >
